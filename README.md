...
その後、未経験からエンジニアの経験を積むために株式会社フィクラ通信技術研究所へ入社し、ネットワークエンジニアとしてネットワークの構築、運用、保守を行いました。
...
また開発スピードを上げるため、Terraformを学び、インフラの運用保守の作業を自動化するなど、業務効率化のために日々技術のキャッチアップを行っています。
...
## シフトプラス株式会社での業務経歴
### SDPFからAWSへの移行　(2024年4月〜2025年3月　私は10月から途中参加)
【プロジェクト概要】
- amazonのふるさと納税参入でAmazonとの連携が必要になる。現環境のままでは連携要件を満たせないためAWSに移行する。
- 移行に必要になりそうなリソースの調査
- 調査内容
  - DMSが名前解決できるようにRoute53にAレコードを追加する
    - bashでcli53 コマンドをループ実行しAレコードを追加する
  - DataSyncでファイルを移行した場合ファイルのパーミッションが維持されるのか検証する
    - 実際にテスト環境を作成し、ファイルを移行してパーミッションが維持されるか確認する。パーミッションは維持された。
  - 作業ミスがでないように1000台以上あるEC2サーバーのホスト名を効率よく変更する
    - AWS CLI でAWS Systems Manager を実行し、ホスト名が変更されるか検証する。
    - 多数あるEC2のhost名を変更するためにBashを利用する。リスト内は$VAR1がインスタンスID、$VAR2が自治体名でしています。
```
#!/bin/bash
cat list.txt | while read VAR1 VAR2
do
    echo VAR1=$VAR1
    echo VAR2=$VAR2
    aws ssm send-command --instance-ids $VAR1 --document-name "AWS-RunShellScript" --comment "change hostname" --parameters "commands=['sudo hostnamectl set-hostname test-$VAR2']"
done
```
【発揮したバリュー】
- これまで調査してきた経験を活かし、迅速に調査し、結果をまとめ、実作業に導入した。

### AWSインフラ基盤の設計/構築　(2024年9月〜2024年1月)
【プロジェクト概要】
- 消防署での勤怠管理システムのAWSインフラ基盤の設計/構築
